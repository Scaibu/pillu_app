...
String formatBytes(final int size, [final int fractionDigits = 2]) {
  if (size <= 0) {
    return '0 B';
  }
...
Color getUserAvatarNameColor(final types.User user, final List<Color> colors) =>
    colors[user.id.hashCode % colors.length];

/// Returns user initials (can have only first letter of firstName/lastName or both).
String getUserInitials(final types.User user) {
  String initials = '';
...
List<Object> calculateChatMessages(
  ...
  final DateFormat? dateFormat,
  required final int dateHeaderThreshold,
  final bool dateIsUtc = false,
  final String? dateLocale,
  required final int groupMessagesThreshold,
  final String? lastReadMessageId,
  required final bool showUserNames,
  final DateFormat? timeFormat,
}) {
  ...
  for (int i = messages.length - 1; i >= 0; i--) {
    ...
    if (showUserNames) {
      final types.Message? previousMessage = isFirst ? null : messages[i + 1];
    ...
    chatMessages.insert(0, <String, Object>{
      'message': message,
